...
class GenerateIconifyPlugin extends Task {
	...
	private $possiblePaths = array ( '' , 'acf' , 'acf-1.0' , 'acf-1.1' ) ;
	...
	function getOptions ()
	{
		...
		{
			if ( $this->futil->hasSubdirs (ROOT.$this->project->name , array ( $path ) ) 
				&& $this->futil->dirExists ( ROOT.$this->project->name.DS.$path.DS.'plugins'.DS.'iconify' ) == true )
			{
				$this->ACFRoot = ROOT.$this->project->name.DS.$path ;
			}
		}
