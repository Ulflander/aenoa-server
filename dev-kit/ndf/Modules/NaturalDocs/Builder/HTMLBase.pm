...
use constant MENU_GROUP_LENGTH => 1; # because it's a line and a blank space
...
sub BuildTitle #(sourceFile)
	...
	my ($name,$path,$suffix) = fileparse($sourceFile);
    my $title = $name;
...
sub BuildMenu #(FileName sourceFile, TopicType indexType) -> string htmlMenu
    ...
    if (!exists $prebuiltMenus{$outputDirectory})
        ...
        $prebuiltMenus{$outputDirectory} = $searchOutput . $titleOutput . $segmentOutput ;
        $output .= $searchOutput . $titleOutput . $segmentOutput ;
        }
    else
