...
class AeAutoTable {
    ...
    function getNextPageLink($page, $length, $count, $order, $dir, $currentWidget, $context) {
        ...
        if ($page < $length) {
            if ($context == 'readFilter' && isset($currentWidget)) {
                ...
                $this->_result[] = '<li><a id="next" class="bold" href="' . $this->getURL() . 'readAll/' . ($page + 1) . (!is_null($order) ? '/' . $order . '/' . $dir . '/false/' : '' ) . '">' . _('Next page') . '</a></li>';
        }
    }
    ...
    function getPreviousPageLink($page, $length, $count, $order, $dir, $currentWidget, $context) {
        if ($page > 1) {
            if ($context == 'readFilter' && isset($currentWidget)) {
                $this->_result[] = '<li><a id="prev" class="bold" href="' . $this->getURL() . 'readAll/' . ($page - 1) . (!is_null($order) ? '/' . $order . '/' . $dir . '/true/' : '' ) . '">' . _('Previous page') . '</a></li>';
            } else {
