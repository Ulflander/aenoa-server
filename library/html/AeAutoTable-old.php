...
class AeAutoTable {
	...
	function startTable($page=1, $length= 1, $count=1, $order='asc', $dir='', $currentWidget=null, $context=null) {
		...
		if ($context != 'readFilter' && isset($currentWidget)) {
			...
			$this->_result[] = '<li><a href="' . $this->getURL() . 'reset-filter/inline' . (!is_null($order) ? '/'.$order.'/' . $dir  .'/false/' . $currentWidget : '' ) . '" class="icon16 rows-list unlabeled inline" id="reset" title="' . _('Reset result') . '"></a></li>';
		}
